...
def data_analysis_page(file_path):
    ...
    selected_column_hist = st.selectbox("Select Numeric Column for Histogram", data.columns)

    # Generate histogram
    hist = alt.Chart(converted_data).mark_bar().encode(
    ...
    data['Availability'] = data.apply(lambda row: 'Unavailable' if row.isna().any() else 'Available', axis=1)
    available_data = data[data['Availability'] == 'Available']
    unavailable_data = data[data['Availability'] == 'Unavailable']
